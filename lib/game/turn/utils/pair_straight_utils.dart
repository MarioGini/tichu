...
List<TichuTurn> getPairStraights(List<Card> cards, int desiredLength) {
  ...
  var pairCards = cards
      .where((element) => occurrenceCount[element.face] == 2)
      .toList();
  ...
  var pairIndices = findConnectedCards(
    pairCards.where((element) => pairCards.indexOf(element) % 2 == 0).toList(),
  ).map((e) => ConnectedCards(2 * e.beginIdx, 2 * e.endIdx)).toList();
