...
- `--rl-policy-team=0|1|all`: apply the policy to one partnership for mixed-team evaluation, or all seats for self-play.
- `--rl-stats-output=stats.json`: write actual policy invocations, selections, state hits, and heuristic fallbacks.
...
- structured `action` plus exact, shape, and actor-relative `policy_action_key`
- `legal_action_keys` when enumerable, `action_index`, diagnostic `reward`, team-score `learning_reward`, `done`, `discount`

The policy comparison tool evaluates learned and heuristic partnerships head
to head, then swaps seats on the same seeds. See [tool/rl/README.md](tool/rl/README.md)
for iterative training, conservative policy construction, and coverage metrics.
