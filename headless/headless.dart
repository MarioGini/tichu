...
class _HeadlessRunner {
  ...
  void _emitAiAction(GameSnapshot snapshot, GameSnapshot previous) {
    if (!snapshot.aiAwaitingConfirmation ||
        snapshot.pendingAiPlayerId == null) {
      return;
    }
    if (previous.aiAwaitingConfirmation &&
        previous.pendingAiPlayerId == snapshot.pendingAiPlayerId &&
        _cardsEqual(previous.pendingAiCards, snapshot.pendingAiCards) &&
        previous.pendingAiPass == snapshot.pendingAiPass) {
      return;
    }
    ...
    _writeRow(
      event: 'ai_action',
      snapshot: snapshot,
      playerId: snapshot.pendingAiPlayerId,
      action: snapshot.pendingAiPass ? 'pass' : 'play',
      cards: snapshot.pendingAiCards,
    );
  }
  ...
  void _maybeConfirmAi(GameSnapshot snapshot) {
    if (!snapshot.aiAwaitingConfirmation) {
      return;
    }
    if (snapshot.pendingAiPlayerId == null) {
      return;
    }

    backend.submitAction(
      gameId,
      ConfirmAiTurnAction(playerId: snapshot.pendingAiPlayerId!),
    );
  }
...
void _printUsage() {
  stdout.writeln('Headless Tichu AI runner');
  stdout.writeln('Usage: dart run headless/headless.dart [--seed=N]');
  stdout.writeln('       [--target-score=N] [--output=game.csv]');
}

List<GamePlayer> _buildAiPlayers() {
  return const [
    GamePlayer(id: 'ai_1', name: 'AI 1', seat: 0, type: PlayerType.ai),
    GamePlayer(id: 'ai_2', name: 'AI 2', seat: 1, type: PlayerType.ai),
    GamePlayer(id: 'ai_3', name: 'AI 3', seat: 2, type: PlayerType.ai),
    GamePlayer(id: 'ai_4', name: 'AI 4', seat: 3, type: PlayerType.ai),
  ];
}
